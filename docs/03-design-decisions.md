...
**Related:** [Calculations](01-calculations.md) | [Architecture](02-architecture.md) | [Final design](04-final-design.md) | [Circuit walkthrough](05-circuit-walkthrough.md)
...
`MN_RST_SW` is a wide switch controlled by shared reset. `MN_RST_LIM` is a longer, smaller device controlled by `VSPIKELEN`. This ensures the bias device, rather than a voltage-dependent switch resistance, primarily sets discharge current.

## D9. Use longer channels for analog current-setting devices

`MN_RST_LIM` uses 0.18 um channel length and the weak `MP_REQ_PU` pull-up uses 0.36 um. Longer channels improve output resistance and matching. Digital inverters and switches use 0.06-0.09 um to maintain speed.
...
## D15. Keep the paper and retarget specifications distinct

The source PDF is licensed reference material. Its topology and equations motivate this design, but its reported 0.6 um process values are not GPDK45 construction values. Every derived 1 V parameter is identified as calculated or calibration-pending.

## D16. Define “final” through measurable criteria

The topology and nominal device table are final for Revision 1.1. Process-dependent values are final only after the Spectre, corner, Monte Carlo, DRC/LVS, and extracted-view criteria in the final-design document pass.

## Revision notes

- **1.1 - 2026-09-17:** Replaced opaque device references with functional names and clarified the source-paper boundary.
